...
const StatChartCard = ({
  label, icon, chartLabels, chartData, chartDatasets, summary, minY, maxY, useLegend = false,
  callback = (item) => addCommasToNumbers(item.raw), style = {}
}) => {
  return <StatChartBox style={style}>
    <StatChartHeader>
      <StatChartHeaderLeft>
...
const stringToMegabytesWithCommas = (value) => addCommasToNumbers(stringToMegabytes(value));
  ...
  return <StatContainer>
    <StatCardsContainer>
      ...
      <StatChartCard
        ...
        summary={<FractionSummary value={stringToMegabytesWithCommas(mem.usedMem)} max={stringToMegabytesWithCommas(mem.totalMem)} />}
        minY={0} maxY={stringToMegabytes(mem.totalMem)}
      />
      <StatChartCard
        label='JVM 메모리(MB)'
        icon={<ComputerChipIcon />}
        ...
        summary={<FractionSummary value={stringToMegabytesWithCommas(jvm.usedMemory)} max={stringToMegabytesWithCommas(jvm.totalMemory)} />}
        minY={0} maxY={stringToMegabytes(jvm.totalMemory)}
      />
    </StatCardsContainer>
    <StatCardsContainer>
      <StatChartCard
        ...
        summary={<FractionSummary value={stringToMegabytesWithCommas(disk.freeSpace)} max={stringToMegabytesWithCommas(disk.totalSpace)} />}
        minY={0} maxY={stringToMegabytes(disk.totalSpace)}
        style={{
          flex: '1 0 0'
        }}
      />
      <StatChartCard
        ...
        summary={<FractionSummary value={floorDecimal(tps[0], 2)} max={'20.00'} />}
        minY={0} maxY={20}
        useLegend={true}
