...
import { PardonIcon } from '../assets/24x-icons';
import { banListState, currentProfileState } from '../contexts/states';
...
const PlayerInfoContainer = ({ uuid, name }) => {
  const [players] = useRecoilState(banListState);
  const player = players[uuid];
  const profile = useRecoilValue(currentProfileState);

  return player ? (
    <PlayerContainer>
      <HeadDisplay $src={`https://mc-heads.net/avatar/${uuid}/46.png`} />
      <PlayerDataDisplay
        topText={uuid}
        bottomText={name}
...
const BanList = () => {
  ...
  const playerDetails = useRecoilValue(banListState);
  ...
  return (
    <PlayersContainer>
      <DashboardSummary label='밴된 플레이어 수' value={players.length} />
